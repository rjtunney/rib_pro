...
double moveToRightQ( vector<int> lattice, vector<int> mrna, int trna, int pos, int w){
	...
	if ( occ == 0 && trna == 1){
		for ( int j = rightMaxPos+1; j <= min((int) lattice.size()-1,rightMaxPos+2); ++j ){
			fold += mrna.at(j);
		}
		if (fold == 0 ){
			return 0.2;
...
burnInOutput burnIn( int prot4BurnIn, int L, double alpha, double beta, vector<double> gamma, vector<int> delta, double refold, int w){

	int nProt = 0; // how many proteins are made

	vector<int> lattice(L,0); // mRNA is empty to start
	vector<int> mrna(L,0); // mRNA is folded to start
	vector<int> occupiedPos(0,0);
	vector<int> unfoldedPos(0,0);
	vector<int> ribState(0,0);
	
	while (nProt < prot4BurnIn){
		int prevLast = lattice.at(lattice.size() - 1);
		singleStepOutput result = simOneTimeStep(lattice, mrna, unfoldedPos, occupiedPos, ribState, alpha, beta, gamma, delta, refold, w);
	...
	burnOut.mrna = mrna;
	burnOut.foldpos = unfoldedPos;
	
	return burnOut;
}


simOutput simulation( ofstream &outfile, int maxTime, vector<int> occupiedPos, vector<int> mrna, vector<int> unfoldedPos, vector<int> ribState, vector<int> lattice, double alpha, double beta, vector<double> gamma, vector<int> delta, double refold, int w){
...
int main(int argc, char* argv[]){
	...
	double alpha = atof(argv[3]); // initiation rate
	double beta = 1; //35; // termination rate
	double refold = 0.005; // refold rate
	...
	for (int n=0; n < num_runs; ++n){
		...
		burnInOutput burnOut = burnIn(prot4BurnIn, L, alpha, beta, gamma, delta, refold, w);
		vector<int> lattice = burnOut.lattice;
		vector<int> occupiedPos = burnOut.pos;
		vector<int> ribState = burnOut.states;
		vector<int> mrna = burnOut.mrna;
		vector<int> unfoldedPos = burnOut.foldpos;
		...
		simOutput simOut = simulation(outfile, maxTime, occupiedPos, mrna, unfoldedPos, ribState, lattice, alpha, beta, gamma, delta, refold, w);
